...
const customerColumn = ({ t, i18n }) => [
  ...
  },
  {
    ...
    render: (createdAt) =>
      i18n.language === "ar"
        ? toArabicDigits(utcDateToLocal(createdAt))
        : utcDateToLocal(createdAt),
  },
];
          ...
          items: [
            {
              label: (
                <NavLink
                  onClick={(e) => {
                    e.preventDefault();
                    navigate("/staff/staffmanagement/editstaff/" + row?.id);
                  }}
                >
                  {t("Edit")}
                </NavLink>
            ...
            row?.isActive === true && {
              label: (
                <NavLink
                  onClick={(e) => {
                    e.preventDefault();
                    setStatusChange({ id: row?.id, status: row?.isActive });
                  }}
                >
                  {t("Inactive")}
                </NavLink>
...
const bookingColumn = ({ t, i18n }) => [
  ...
  },
  {
    ...
    dataIndex: "service",
    render: (service) => {
      return <Tag className="sm-pill radius-20 fs-12">{service?.name}</Tag>;
    },
  },
  {
  ...
  },
  {
    ...
    render: (status) => {
      return status === "COMPLETED" ? (
        ...
        <Text className="btnpill fs-12 dsasellerpending">
          {t("In-Progress")}
        </Text>
      ) : (
...
const revenueColumns = ({ t, i18n }) => [
  {
    ...
    render: (businessId) =>
      `${businessId ? `#${businessId?.slice(0, 5)}` : "-"}`,
  },
  {
      ...
      business?.businessType
        ? t(
            business?.businessType?.charAt(0)?.toUpperCase() +
              business?.businessType?.slice(1)?.toLowerCase(),
          )
        : "-",
  },
  {
    ...
    render: (subscription) => {
      return subscription?.type === "BASIC" ? (
        <Text className="sm-pill text-white fs-12 bg-basic-color">
          {t("BP")}
        </Text>
        ...
        <Text className="sm-pill text-white fs-12 bg-apple-green">
          {t("EP")}
        </Text>
      ) : null;
    },
  },
  {
    ...
    dataIndex: "validity",
    render: (validity) =>
      t(validity?.charAt(0)?.toUpperCase() + validity?.slice(1)?.toLowerCase()),
  },
  {
    title: t("Discount"),
        ...
        <Flex align="flex-end" gap={5}>
          ...
          {row?.validity === "MONTHLY" ? (
            row?.subscription?.discountPrice > 0 ? (
              <>
                {isArabic
                  ? toArabicDigits(row?.subscription?.discountPrice)
                  : row?.subscription?.discountPrice}
                <sup>
                  <Text className="fs-12" delete>
                    {isArabic
                      ? toArabicDigits(row?.subscription?.price)
            ...
            <>
              {isArabic
                ? toArabicDigits(row?.subscription?.discountYearlyPrice)
                : row?.subscription?.discountYearlyPrice}
              <sup>
                <Text className="fs-12" delete>
                  {isArabic
                    ? toArabicDigits(row?.subscription?.yearlyPrice)
                    : row?.subscription?.yearlyPrice}
                </Text>
              </sup>
            </>
          ) : (
  ...
  },
  {
    ...
    render: (createdAt) =>
      i18n.language === "ar"
        ? toArabicDigits(utcDateTimeToLocal(createdAt))
        : utcDateTimeToLocal(createdAt),
  },
];
      ...
      return status === "PENDING" ? (
        ...
        >
          <Flex
            ...
            className="btnpill fs-12 dsasellerpending cursor w-100px"
          >
        ...
        <Text className="btnpill fs-12 success">
          {t(capitalizeTranslated(formattedStatus))}
        </Text>
      );
    },
...
const discountColumns = ({
    ...
    render: (group) => {
      return (
        <Tag className="sm-pill radius-20 fs-12">
          {t(group?.charAt(0)?.toUpperCase() + group?.slice(1).toLowerCase())}
        </Tag>
      );
    },
  },
    ...
    render: (discountType) =>
      t(
        discountType?.charAt(0)?.toUpperCase() +
          discountType?.slice(1).toLowerCase(),
      ),
  },
  {
    title: t("Value"),
            ...
            <Tag className="sm-pill radius-20 fs-12" key={items?.id}>
              {t(
                items?.type?.charAt(0)?.toUpperCase() +
                  items?.type?.slice(1).toLowerCase(),
              )}
            </Tag>
          ))}
        </Flex>
    ...
    render: (_, row) => {
      const usageLimit =
        row?.usageLimit === undefined || row?.usageLimit === null
          ...
          : row?.usageLimit;
      const usageCount =
        row?.usageCount === undefined || row?.usageCount === null
          ? 0
          : row?.usageCount;
  ...
  },
  {
    ...
    render: (_, row) => {
      ...
      return (
        <Dropdown menu={{ items }} trigger={["click"]} disabled={!allowActions}>
          <Button
        ...
        {userRole.includes("SUPER_ADMIN")
          ...
          : userRole.includes("SERVICE_PROVIDER")
            ...
            : userRole.includes("STAFF_MEMBER")
              ? t("Staff Manager")
              : userRole.includes("Demo_Admin")
                ? t("Demo Admin")
                : userRole.includes("Technical_Admin")
                  ? t("Technical Admin")
  ...
  },
  {
    ...
    render: (subscription) => {
      return subscription?.type === "BASIC" ? (
        ...
        <Text className="sm-pill text-white fs-12 bg-violet">
          {t("Standard Plan")}
        </Text>
      ) : subscription?.type === "PRO" ? (
        <Text className="sm-pill text-white fs-12  bg-red">
          {t("PRO Plan")}
        </Text>
      ) : (
        <Text className="sm-pill text-white fs-12 bg-apple-green">
            ...
            row?.status === "ACTIVE" && {
              label: (
                <NavLink
                  onClick={(e) => {
                    e.preventDefault();
                    navigate("/allbusiness/viewbusiness/" + row?.id);
                  }}
                >
                  {t("View")}
                </NavLink>
              ...
              ? {
                  label: (
                    <NavLink
                      onClick={(e) => {
                        e.preventDefault();
                        setStatusChange({ id: row?.id, status: "ACTIVE" });
                      }}
                    >
                      {t("Active")}
                    </NavLink>
              ...
              : {
                  label: (
                    <NavLink
                      onClick={(e) => {
                        e.preventDefault();
                        setStatusChange({ id: row?.id, status: "INACTIVE" });
                      }}
                    >
                      {t("Inactive")}
                    </NavLink>
...
const businessserviceColumns = ({ t, i18n }) => [
  ...
  },
  {
    title: t("Price"),
    dataIndex: "price",
    render: (price) => (i18n.language === "ar" ? toArabicDigits(price) : price),
  },
  {
    title: t("Status"),
    dataIndex: "status",
...
const businessstaffColumns = ({ t }) => [
  ...
  },
  {
    ...
    dataIndex: "firstName",
    render: (_, row) => (
      <FieldMerger object={row} fields={["firstName", "lastName"]} />
    ),
  },
  {
    title: t("Phone Number"),
      ...
      return (
        <Flex gap={5} wrap>
          {services?.map((items, index) => (
            <Tag key={index} className="sm-pill radius-20 fs-12">
              {items?.name}
            </Tag>
          ))}
        </Flex>
