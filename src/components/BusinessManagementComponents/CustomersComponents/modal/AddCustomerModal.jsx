...
const AddCustomerModal = ({ visible, onClose, refetch }) => {
  ...
  const [createSubscriberCustomer, { loading }] = useMutation(
    CREATE_SUBSCRIBER_CUSTOMER,
    {
      onCompleted: () => {
        notifySuccess(
          api,
          t("Customer Create"),
          t("Customer has been created successfully"),
        );
        refetch();
        onClose();
        form.resetFields();
  ...
  const AddEditSubsriberCustomer = async () => {
    ...
    try {
      await createSubscriberCustomer({
        variables: { input: { ...data, role: "SUBSCRIBER" } },
      });
    } catch (e) {
      console.error(e);
    }
