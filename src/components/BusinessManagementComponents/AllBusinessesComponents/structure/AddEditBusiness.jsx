...
const AddEditBusiness = () => {
  ...
  const [selectedSubscriptionPlan, setSelectedSubscriptionPlan] =
    useState(null);
  const [getVerifyDiscount, { loading: verifyingDiscount }] = useLazyQuery(
    VERIFY_DISCOUNT,
    {
      fetchPolicy: "network-only",
    },
  );
  const [createBusiness, { loading, error, success }] = useMutation(
    CREATE_BUSINESS,
    {
      onCompleted: () => {
        notifySuccess(
          api,
          t("Business Create"),
          t("Business has been created successfully"),
          () => {
  ...
  const [getSubscriberCustomersLookup, { data }] = useLazyQuery(
    GET_SUBSCRIBER_CUSTOMERS_LOOKUP,
    {
      fetchPolicy: "network-only",
    },
  ...
  const onFinish = async () => {
    ...
    data = {
      ...data,
      image: previewimage,
      subscriberId: subscriberCustomersLookup?.find(
        (subscriber) => subscriber?.id === data?.subscriberId,
      )?.id,
      ...
      subscriptionPrice:
        selectedSubscriptionPlan?.type === "ENTERPRISE"
          ...
          : (discountData?.finalPrice ?? subscriptionPrice),
  ...
  const checkDiscountCode = async () => {
    ...
    try {
      setDiscountData(null);
      const res = await getVerifyDiscount({
        variables: {
          code: data?.code,
          subscriberId: subscriberCustomersLookup?.find(
            (subscriber) => subscriber?.id === data?.subscriberId,
  ...
  return (
    <>
      ...
      <Flex vertical gap={10}>
        <BreadCrumbCard
          items={[{ title: title }, { title: t("All Businesses") }]}
        />
              ...
              <Row gutter={16}>
                ...
                <Col span={24} md={12}>
                  <MySelect
                    ...
                    disabled={
                      !subscriberCustomersLookup?.length ||
                      !subscriberCustomersLookup
                    }
                ...
                <Col span={24} md={12}>
                  <MyInput
                    ...
                    addonBefore={
                      ...
                      >
                        <Select value="sa">+966</Select>

                        <Select value="ae">+971</Select>
                      </Select>
                    }
                    ...
                    suffix={
                      <Flex align="center" gap={2}>
                        ...
                        {discountstatus !== null &&
                          !verifyingDiscount &&
                          (discountstatus ? (
                            <Text className="text-green fs-12">
                              {t("Valid")}
                            </Text>
                          ) : (
                            <Text className="text-red fs-12">
                              {t("Invalid")}
                            </Text>
                          ))}
                        <Tag onClick={checkDiscountCode} className="cursor">
                          {t("Check")}
                        </Tag>
                      </Flex>
                    }
                ...
                <Col span={24}>
                  <Title level={5} className="fw-500 my-3">
      ...
      <ConfirmModal
        ...
        title={"Are you sure?"}
        subtitle={
          "Please confirm that all business details are correct and a subscription plan is assigned before proceeding."
        }
