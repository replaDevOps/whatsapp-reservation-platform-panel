...
const AddEditStaff = () => {
  ...
  const [createstaff, { loading: creating }] = useMutation(CREATE_STAFF, {
    onCompleted: () => {
      notifySuccess(
        api,
        t("Staff Create"),
        t("Staff has been created successfully."),
        () => {
          navigate("/staff");
        },
      );
  ...
  const [updatestaff, { loading: updating }] = useMutation(UPDATE_STAFF, {
    onCompleted: () => {
      notifySuccess(
        api,
        t("Staff Update"),
        t("Staff has been updated successfully."),
        () => {
          navigate("/staff");
        },
      );
  ...
  const { data, loading, refetch } = useQuery(GET_STAFFS_BY_ID, {
  ...
  return (
    <>
      ...
      <Flex vertical gap={10} className="position-relative">
        <BreadCrumbCard items={[{ title: title }, { title: t("Staffs") }]} />
        {loading ? (
