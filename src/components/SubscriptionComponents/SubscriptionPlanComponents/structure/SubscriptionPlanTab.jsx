...
const SubscriptionPlanTab = ({ setState }) => {
  ...
  const [getSubscriptionPlans, { data, loading }] = useLazyQuery(
  ...
  useEffect(() => {
    if (getSubscriptionPlans) getSubscriptionPlans();
  ...
  useEffect(() => {
    if (data?.getSubscriptions?.length) {
      ...
      setActiveSubscriptionPlan({
        ...data?.getSubscriptions?.find((plan) => plan?.type === items[0]?.key),
        title: items[0]?.key,
      });
    }
  }, [data]);
  const onChange = (key) => {
    setActiveKey(key);
    setActiveSubscriptionPlan({
      ...subscriptionPlans?.find((plan) => plan?.type === key),
      title: key,
    });
  };
  return (
        ...
        <Flex vertical gap={10}>
                ...
                <Title level={5} className="m-0">
                  {t(
                    capitalizeTranslated(
                      activeSubscriptionPlan?.title + " " + "Subscription Plan",
                    ),
                  )}
                </Title>
                <Button
                  className="btncancel"
                  onClick={() => {
                    setEditItem(activeSubscriptionPlan);
                    setState("Edit Subscription Plan");
                  }}
                >
                  <img
                    src="/assets/icons/edit-b.webp"
                    width={20}
                    alt="edit icon"
                  />{" "}
                  {t("Edit Subscription Plan")}
          ...
          <Card className="card-bg card-cs radius-12 border-gray">
            {edititem ? (
              ...
              <SubscriptionPlanCard subscriptionPlan={activeSubscriptionPlan} />
            )}
          </Card>
        </Flex>
      </Col>
